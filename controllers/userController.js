...
exports.signup = BigPromise(async (req, res, next) => {
  ...
  if (!name || !email || !password) {
    return next(new CustomError("Name, Email and Password are must", 400));
  }
...
exports.forgotPassword = BigPromise( async (req, res,next) =>  {
  ...
  const forgotToken = user.getForgotPasswordToken();
   ...
   // to save token and expiry date generated in getForgotPasswordToken() method in the db 
   // save user field in db
   // NOTE : token saved in db is encrypted
  await user.save({validateBeforeSave : false});

  // create a URL and this url contains token for password reset which is sent through mail(nodemailer aka mailHelper({}))
  ...
  try {
    ...
    // if sending the email fails, we have to clear forgotPasswordToken and forgotPasswordExpiry and save.
...
exports.changePassword = BigPromise(async (req, res, next) => {
  ...
  if(!isPasswordCorrect){
    return next(new CustomError("old password entered is incorrect", 401));
  }
...
exports.updateUserDetails = BigPromise(async (req, res, next) => {
  ...
  // We can add a check for name and email in the body
  ...
  // id user sends any photo to update
  if(req.files){

    // get user by user id
    const user = await User.findById(req.user.id);

    // get image id of already existing photo from db. here photo is an object in db containing attrubutes id and secure_url
    ...
    // update new image details in db
    newData.photo = {
      id : result.public_id,
      secure_url :  result.secure_url
    };
...
exports.adminGetOneUser = BigPromise(async (req, res, next) => {
  ...
  if(!user){
    return next(new CustomError('User Id does not exist', 400));
  }
